...
file_put_contents('/var/www/bbs/storage/logs/log.txt', var_export($matches,true), FILE_APPEND);
		foreach ($matches as $m)
		{
			...
			if (isset($m[5]))
			{
                ...
                elseif ($m[5][0][0] === ':')
                {
                    ...
                    if (isset($codeTag) && $m[5][0] === $codeFence)
                    {
                        $codeTag->pairWith($this->parser->addEndTag('ANSWER', $tagPos, $tagLen, -1));
                        $this->parser->addIgnoreTag($textBoundary, $tagPos - $textBoundary);
                    ...
                    {
                        // Create code block
                        $codeTag   = $this->parser->addStartTag('ANSWER', $tagPos, $tagLen);
                        $codeFence = substr($m[5][0], 0, strspn($m[5][0], ':~'));
                        $codeTag->setAttribute('blockDepth', $blockDepth);
                ...
                elseif ($m[5][0][0] === '%')
                {
                    ...
                    if (isset($codeTag) && $m[5][0] === $codeFence)
                    {
                        $codeTag->pairWith($this->parser->addEndTag('QUESTION', $tagPos, $tagLen, -1));
                        $this->parser->addIgnoreTag($textBoundary, $tagPos - $textBoundary);
                    ...
                    {
                        // Create code block
                        $codeTag   = $this->parser->addStartTag('QUESTION', $tagPos, $tagLen);
                        $codeFence = substr($m[5][0], 0, strspn($m[5][0], '%~'));
                        $codeTag->setAttribute('blockDepth', $blockDepth);
