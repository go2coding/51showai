...
class BlocksAnswer extends AbstractPass
{
	/**
	* @var array
	*/
...
file_put_contents('/var/www/bbs/storage/logs/log.txt', "matches:".var_export($matches,true), FILE_APPEND);
...
file_put_contents('/var/www/bbs/storage/logs/log.txt', "addStartTag List", FILE_APPEND);
			...
			if (isset($m[5]))
			{
				...
				elseif ($m[5][0][0] === ':' || $m[5][0][0] === '~')
				{
					...
					$tagLen = $lfPos - $tagPos;
file_put_contents('/var/www/bbs/storage/logs/log.txt', "m5".var_export($m[5],true).'codeFence:'.var_export($codeFence,true), FILE_APPEND);
...
file_put_contents('/var/www/bbs/storage/logs/log.txt', " Code fence addEndTag", FILE_APPEND);
						// Overwrite the whole block
